import { getDepopProductsFromUser } from './depop.js'
import db from './db';
import Product from '../models/product';
import fetch from 'node-fetch';

const dotenv = require('dotenv').config();
const storeId = process.env.STOREID;
let updateInProgress = false;

async function addNewProductsToDB(storeId) {
	await getDepopProductsFromUser(storeId)
		.then(products => {
			products.forEach(async product => {
				const { id, slug, sold } = product;
				const match = await findProductByID(id);
				if (match === null) {
					const price = product.price.price_amount;
					const productData = {
						id,
						slug,
						sold,
						price,
					}
					await addNewProductToDatabase(productData);
				}
			})
		});
}

async function addDetailsToProducts() {
	const products = await getProductsWithoutDetails();
	products.forEach(async product => {
		const details = await getProductDetailsFromSlug(product.slug);
		console.log(details);
	})
}

export async function updateProducts() {
	updateInProgress = true;
	// await addNewProductsToDB(storeId)
	await addDetailsToProducts();
	updateInProgress = false;
}

async function getProductDetailsFromSlug(slug) {
	const baseUrl = 'https://popgoblin.kevinforrestkeyes.now.sh/';
	const url = `${baseUrl}${slug}`;
	const details = await fetch(url)
		// .then(res => res.json())
		.then(res => {
			// try {
			// 	res.json();
			// } catch (e) {
			// 	console.log("<<<<-----ERROR----->>>");
			// 	console.log(e);
			// 	console.log(res);
			// }
			if (url.includes('-vintage-90s-gap-army-green')) {
				console.log(res);
			}
		})
		.catch(function(err) {
			console.error(err);
		})
	return details;
}

export function getUpdateStatus() {
	return { updateInProgress };
}

async function addNewProductToDatabase(productData) {
	const product = new Product();
	const { id, slug, sold, price } = productData;
	product.id = id;
	product.slug = slug;
	product.price = price;
	product.sold = sold;
	product.details = false;
	product.save(err => {
		if (err) {
			console.error(err);
		}
	})
	return { status: 'success' };
}

export async function getProducts() {
	const products = Product.find().exec();
	return products;
}

export async function getProductsWithoutDetails() {
	const products = Product.find({ 
		details: false, 
		sold: false 
	});
	return products;
}

export async function findProductByID(id) {
	const product = Product.findOne({
    id: id
	});
	return product;
}